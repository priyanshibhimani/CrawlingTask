...
//        //       bool result = GetAuctionDataByAuctionID(model.Id);
...
//                return;  // Exit if no auction nodes are found
//            }

//            Console.WriteLine($"Found {auctionsNodes.Count} auction nodes.");

//            // Iterate over each auction node and insert its data
//            foreach (HtmlNode node in auctionsNodes)
//            {
//                AuctionModel model = new AuctionModel();
//                SetModelData(node, model);
//                InsertAuctionIntoDatabase(model);
//                //bool exists = GetAuctionDataByAuctionID(model.Id);

//                //if (!exists)  // Insert only if the auction does not exist
//                //{
//                //    Console.WriteLine($"Inserting auction with ID: {model.Id}");
//                //    InsertAuctionIntoDatabase(model);
//                //}
//                //else
//                //{
//                //    Console.WriteLine($"Auction with ID: {model.Id} already exists.");
//                //}
//            }
//        }
...
//                // Select the <a> tag within the titleNode
//                //HtmlNode Description = DescriptionXPathNode.SelectSingleNode(".//a");
...
//                // Get the inner text of the anchor node
...
//                string pattern1 = @"\d{1,}"; // Match one or more digits
//                Regex regex = new Regex(pattern1);
//                Match match = regex.Match(t);

//                if (match.Success)
//                {
//                    // Extract the lot size and set it to model
...
//            try
//            {
//                // Updated XPath to be specific to the current auction node
...
//                // Use SelectSingleNode to get the first matching node specific to the current auction
...
//                string pattern1 = @"^\d{1,2}"; // Match one or more digits
//                Regex regex = new Regex(pattern1);
//                Match match = regex.Match(t);
...
//                string pattern1 = @"^(\s+)?\d+\s([A-z]+)"; // Match one or more digits
//                Regex regex = new Regex(pattern1);
//                Match match = regex.Match(t);
...
//                string pattern1 = @"(\d{1,2}\s*-\s*[A-Z]+\s*(\d{4}))|\b(\d{4})\b(?=\s*(-|,))"; // Match one or more digits
//                Regex regex = new Regex(pattern1);
//                Match match = regex.Match(t);
...
//                string pattern1 = @"-\s*(\d+)"; // Match one or more digits
//                Regex regex = new Regex(pattern1);
//                Match match = regex.Match(t);
...
//                string pattern1 = @"-\s*\d+\s[A-Z]+\s(\d+)$"; // Match one or more digits
//                Regex regex = new Regex(pattern1);
//                Match match = regex.Match(t);
...
//                //command.Parameters.AddWithValue("@LotCount", model.LotCount);
...
//                command.Parameters.AddWithValue("@StartYear", model.StartYear ?? (object)DBNull.Value); // Ensure StartYear is handled
...
//        }
//        #endregion
//        public bool GetAuctionDataByAuctionID(string auctionId)
//        {
